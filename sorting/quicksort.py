...
def part_pivot(arr, first, last):
##assign a pivot element and partition the array into 2 splits. value of (Left split elements) < value(pivot element) < value (Right split elements)     
    var_1 = (first-1)         
    pivot = array[last]   
    for var_2 in range(first, last):
        if arr[var_2] <= pivot:
            var_1 = var_1+1
            arr[var_1], arr[var_2] = arr[var_2], arr[var_1]
    arr[var_1+1], arr[last] = arr[last], arr[var_1+1]
    
    return (var_1+1)
      
def quickSort(arr, first, last):
#If array has just one element, return it since no sorting is needed else call the quicksort function to perform the sorting    
    if len(arr) == 1:
        return arr
    if first < last:
        partioningindex = part_pivot(arr, first, last)
        quickSort(arr, first, partioningindex-1)
        quickSort(arr, partioningindex+1, last)
...
if __name__ == '__main__':
    ...
    print("Choose from one of the below 4 options to perform quick sort with last element as pivot:")
# In random_number.py we nhave generated 4 methods to create 4 text files having array of 20,100,1000,4000 elements that ranges from 0 to 6000.
    print("\n 1: Array of 20 elements, 2: Array of 100 elements, 3: Array of 1000 elements, 4: Array of 4000 elements")
...
# Calling the function to perform quick sort
